...
def aux_split(data):
	...
	for track in np.unique(data['track_id']):
		...
		# print(seg_lst, "Number of tracks")
		for seg_id in seg_lst:
			# print("SegId" , (aux_empty_dict['segment_id'] == seg_id) , np.count_nonzero((aux_empty_dict['segment_id'] == seg_id)))
			idx = np.logical_or(idx,(data['segment_id'] == seg_id))
...
class LossAndErrorPrintingCallback(tf.keras.callbacks.Callback):
	def on_epoch_end(self, epoch, logs=None):
		global bst_val_acc
		global best_model
		if (logs['val_accuracy'] > bst_val_acc):
...
# Model fit
history = model.fit(train_x, train_y, batch_size = batch_size, epochs = no_epochs, 
										validation_data = (val_x, val_y),callbacks=[LossAndErrorPrintingCallback()])
